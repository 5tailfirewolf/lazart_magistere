...
The last column "Countermeasures" enumerates the protection used for each version (the protection are the same for version X and it's Rust counter part):
\begin{itemize}
...
\end{itemize}
\todo[inline]{explain different value}

In the VP0 example, we observe 8 missing attacks when we compare to the C result.Since VP0 doesn't have hardened boolean, so it use regular boolean, \texttt{rustc} reuse the branch condition in the \texttt{verify\_pin} function, through xor opterations, to compute its return value, reducing the number of observable injection points on that path. All the higher-order attacks are redundant with those 8 one fault attacks.
...
As shown on the table \ref{tbl:results-fissc-mcmp}, the rust implementation behave exactly as the C implementation, showing the same result for 1 to 4 faults with combination of TI and DL fault models.
